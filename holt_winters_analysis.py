...
def load_data(filename='unique_dates_counts.csv'):
...
def split_data(series, test_size=0.2):
    ...
    Táto funkcia rozdelí časový rad na trénovaciu a testovaciu množinu
    ...
    test_size : float, voliteľný
        Pomer veľkosti testovacej množiny k celkovej veľkosti dát (predvolene 0.2)
    ...
    print(f"\nData split: {train_size} training observations, {len(test)} test observations")
    return train, test
...
def optimize_holt_winters(train, test, seasonal_periods=None):
    ...
    if seasonal_periods is None:
        if pd.infer_freq(train.index) == 'D':
            seasonal_periods = 7  # Weekly seasonality for daily data
        elif pd.infer_freq(train.index) == 'M':
    ...
    for i, (trend, seasonal, sl, ss, ssa) in enumerate(valid_params):
        try:
            ...
            # Fit model
            model = ExponentialSmoothing(train, **model_params)
            ...
            # Evaluate
            metrics = evaluate_model(predictions, test)
...
def test_multiple_seasonal_periods(train, test):
    ...
    max_period = len(train) // 2  # Maximálna perióda by nemala byť väčšia ako polovica dĺžky trénovacích dát
    ...
    for i, period in enumerate(seasonal_periods_to_test):
        ...
        model_params = optimize_holt_winters(train, test, seasonal_periods=period)
...
def main():
    ...
    best_params = test_multiple_seasonal_periods(train, test)
